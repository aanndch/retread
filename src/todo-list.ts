...
export const TODO_SECTIONS: TodoSection[] = [
  {
    ...
    items: [
      ...
      { id: 'url-nav', title: 'URL-reflects-state navigation', status: 'done', note: 'Search/settings are real routes, page modals are `?modal=`, transient dialogs stay local; no more hand-rolled history coordination.' },
      { id: 'context-back', title: 'Context-aware back', status: 'done', note: 'Back returns you where you came from (search/gallery restored), with a logical-parent fallback on deep links.' },
      { id: 'motion-system', title: 'Unified motion system', status: 'done', note: 'One duration-token set, one exit-fade hook, one global reduced-motion gate, consistent overlay envelope.' },
      { id: 'settings-page', title: 'Settings as a routed page', status: 'done', note: '`#/settings` with the standard PageHeader instead of a modal over Home.' },
      { id: 'photo-lightbox', title: 'Shared typewriter photo lightbox', status: 'done', note: 'One mounted-print viewer on Photos/leg/ride: set-as-cover, swipe + keyboard + arrow nav, stable chrome, directional slide animation.' },
      { id: 'heic', title: 'HEIC photo support', status: 'done', note: 'iPhone HEIC converts to JPEG on upload; decoder is lazy-loaded so it costs nothing to the initial bundle until a HEIC is actually used.' },
      { id: 'demo-photos', title: 'Real nature photos in the demo seed', status: 'done', note: 'Varied aspect ratios exercise the lightbox and grid.' },
    ],
  },
  {
    id: 'bugs',
    ...
    items: [
      ...
      { id: 'search-flicker', title: 'Search close flickered (closed, reopened, closed)', status: 'done', note: 'Overlay close raced history; search is now a route, so close is a single native history pop.' },
      { id: 'back-skip', title: 'Back from a result skipped search/gallery', status: 'done', note: 'Context-aware back now returns to where you came from.' },
      { id: 'field-grow', title: 'Search field grew when you started typing', status: 'done', note: 'The clear button no longer dictates the field\'s height.' },
      { id: 'title-clear', title: 'Couldn\'t clear a prefilled ride title', status: 'done', note: 'The auto-derived title moved to the placeholder, so you can genuinely empty the field.' },
      { id: 'map-picker-dark', title: 'Map-picker search input was invisible in dark themes', status: 'done', note: 'Paper-filled input that\'s legible over the dark map tiles in every theme.' },
      { id: 'gallery-jank', title: 'Gallery loaded janky with layout shift', status: 'done', note: 'Synchronous tile wall + reserved geometry; restored the masonry grid.' },
      { id: 'chevron-clip', title: 'Lightbox chevrons looked cut off / off-center', status: 'done', note: 'CSS-drawn chevrons that center reliably in the pager.' },
      { id: 'highlight-reach', title: 'Search highlight didn\'t reach the target note', status: 'done', note: 'Overlap-aware reach check + waiting for the route reveal before scrolling.' },
    ],
  },
  {
    id: 'roadmap',
