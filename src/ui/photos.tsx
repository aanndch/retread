...
// shell-level paper overlay. Thumb object URLs are created here, locally, so
// a big book never holds more URLs than what's on this screen.
export function Photos({ ridesData, photos, onOpenPhoto, onNavigate, onNavigateBack }: PhotosProps) {
        ...
        {ridesData === undefined ? (
          ...
          <div class="photos-masonry">
            {wall.map(({ p, url }) => (
              ...
              >
                <img src={url} alt={p.ride.title || 'Ride photograph'} loading="lazy" />
              </button>
            ))}
          </div>
        )}
